...
class TechnicalAnalysis(object):
    ...
    @staticmethod
    def bollingerBandIndicator(df, close_col, subtitle, n_period=20, n_std=2, fillna=False):
        ...
        col = '%s_bollingerIndicator' % subtitle
        df[col] = 0
    ...
    @staticmethod
    def bollingerBandScaled(df, close_col, subtitle, n_period=20, n_std=2, fillna=False):
    ...
    def rsiIndicator(self, df, close_col, subtitle, n_period=14, upper_thres=65, lower_thres=35, fillna=False):
        ...
        attempt to identify overbought or oversold conditions in the trading of an asset. Returns 1 if exceeding
        upper threshold; -1 if lower; 0 otherwise.
        """
        df = df.copy()
        ...
        col = '%s_rsiIndicator' % subtitle
        df[col] = 100 * emaup / (emaup + emadn)
        df[col] = df[col].apply(lambda r: -1 if r < lower_thres else 1 if r > upper_thres else 0)
        if fillna:
            df[col] = df[col].replace([np.inf, -np.inf], np.nan).fillna(0)
        df[col].hist(bins=30)
        plt.title(col)
